% SYNTAX:
% data2 = hmrR_NirsPlot(snirfData, FreqCut, window, windowOverlap, qualityThreshold, conditionsMask, lambdaMask, dodFlag, guiFlag)
%
% UI NAME:
% hmrR_NirsPlot
%
...
% guiFlag: A flag indicating whether to start or not the GUI(close: 0).
% 
% OUTPUT:
% qualityMatrices - is an structure that includes the set of fields:
%
% USAGE OPTIONS:
% nirsPlot_struct: [qualityMatrices] = hmrR_NirsPlot(snirfData, FreqCut, window, windowOverlap, qualityThreshold, conditionsMask, lambdaMask, dodFlag, guiFlag)
% 
% PARAMETERS:
% FreqCut:[0.5 2.5]
% window: 5;
...
function qualityMatrices = hmrR_NirsPlot(snirfData, FreqCut, window, windowOverlap, qualityThreshold, conditionsMask, lambdaMask, dodFlag, guiFlag)
...
%   --------NirsPlot starts from here---------  
    ...
    % 'qualityThreshold'
    if isfloat(qualityThreshold) && qualityThreshold >= 0 && qualityThreshold <= 1
        q_threshold = qualityThreshold;
    end
...
nirsplot_parameters.qualityThreshold = q_threshold;
...
% calculation of the qualityMatrices
        ...
        qltyThld = nirsplot_param.qualityThreshold;
